...
public class AuthEventHandler {
    ...
    public static void fullSync(ServerPlayer player, boolean auth) {
        ...
        LOGGER.info("完整同步 {}: mayBuild = {}", player.getName().getString(), auth);
    }
    ...
    private static void sendAuthMessage(ServerPlayer player) {
        ...
        player.sendSystemMessage(Component.literal("§e§l登录验证"));
        player.sendSystemMessage(Component.literal(""));
        player.sendSystemMessage(Component.literal("§e请输入："));
        player.sendSystemMessage(Component.literal("   §a/login <密码>  §7或"));
        player.sendSystemMessage(Component.literal("   §a/register <密码>"));
        player.sendSystemMessage(Component.literal(""));
        player.sendSystemMessage(Component.literal("§7(至少4个字符)"));
        player.sendSystemMessage(Component.literal("§6▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
    }

    private static void sendReminder(ServerPlayer player) {
        player.sendSystemMessage(Component.literal("§c请先登录！"));
    }
}